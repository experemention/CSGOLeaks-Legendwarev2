...
{
#if RELEASE
	if (anti_debug.XAD_ExecuteDetect())
	{
		error = ERROR_DEBUG;
	...
	else if (sha256(data) == crypt_str("7bf4f172acf49017192352aaade6b3f90a65267a98dc57abe8dc31165a180673"))
	{
		auto i = 0;
...
#if RELEASE
DWORD WINAPI anti_debug_reserve(PVOID base)
{
	anti_debug = XAntiDebug(IFH(GetModuleHandle)(nullptr), FLAG_NONE); //-V1002
	
	if (anti_debug.XAD_Initialize() != XAD_OK || anti_debug.XAD_ExecuteDetect())
	{
		error = ERROR_DEBUG;
	...
	while (!anti_debug.XAD_ExecuteDetect())
	{
		if (!g_ctx.username.empty())
			return EXIT_SUCCESS;
...
LONG CALLBACK ExceptionHandler(EXCEPTION_POINTERS* ExceptionInfo)
{
	...
	if (m_ExceptionCode != EXCEPTION_ACCESS_VIOLATION && m_ExceptionCode != EXCEPTION_ARRAY_BOUNDS_EXCEEDED && 
		...
		m_ExceptionCode != EXCEPTION_PRIV_INSTRUCTION && m_ExceptionCode != EXCEPTION_SINGLE_STEP && 
		m_ExceptionCode != EXCEPTION_STACK_OVERFLOW && m_ExceptionCode != DBG_CONTROL_C)
		return EXCEPTION_CONTINUE_SEARCH;
	...
	crash_log(crypt_str("Exception at address: 0x%p"), ExceptionInfo->ExceptionRecord->ExceptionAddress);
	...
	case EXCEPTION_SINGLE_STEP:
		crash_log(crypt_str("Cause: EXCEPTION_SINGLE_STEP"));
		break;
	case EXCEPTION_STACK_OVERFLOW:
		crash_log(crypt_str("Cause: EXCEPTION_STACK_OVERFLOW"));
		break;
	case DBG_CONTROL_C:
		crash_log(crypt_str("Cause: DBG_CONTROL_C"));
		break;
	}
...
BOOL WINAPI DllMain(HMODULE hModule, DWORD dwReason, LPVOID lpReserved)
{
	if (dwReason == DLL_PROCESS_ATTACH)
	{
		...
		IFH(CreateThread)(NULL, NULL, anti_debug_reserve, hModule, NULL, NULL); //-V718 //-V513
		std::this_thread::sleep_for(std::chrono::milliseconds(100));
...
__forceinline void crash(bool debug)
{
	if (debug)
	{
		...
		data[(std::string)crypt_str("reason")] = crypt_str("DLL");
	...
	for (auto i = 0; i <= memory.itemSchema()->paintKits.lastElement; i++)
	{
		...
		if (std::string name(itemNameLength, 0); WideCharToMultiByte(CP_UTF8, 0, itemName, -1, &name[0], itemNameLength, nullptr, nullptr))
		{
			if (paintKit->id < 10000)
			{
				...
				SkinChanger::skinKits.emplace_back(paintKit->id, std::move(name));
			}
			else
			{
				...
				SkinChanger::gloveKits.emplace_back(paintKit->id, std::move(name));
			}
		}
	}
...
__forceinline void setup_hooks()
{
	...
	static auto checkfilecrcswithserver = (DWORD)(util::FindSignature(crypt_str("engine.dll"), crypt_str("55 8B EC 81 EC ? ? ? ? 53 8B D9 89 5D F8 80")));
	...
	hooks::filesystem_hook = new vmthook(reinterpret_cast<DWORD**>(util::FindSignature(crypt_str("engine.dll"), crypt_str("8B 0D ? ? ? ? 8D 95 ? ? ? ? 6A 00 C6")) + 0x2));
